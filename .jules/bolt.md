...
## 2025-05-30 - [Fix Scroll Event Debouncing]
**Learning:** When debouncing events (e.g., scroll) using requestAnimationFrame and a ticking flag, resetting the flag synchronously outside the callback or over-nesting requestAnimationFrame calls nullifies the performance benefits and causes layout thrashing.
**Action:** Ensure the ticking flag is reset to false strictly inside the asynchronous callback and avoid over-nesting requestAnimationFrame.
