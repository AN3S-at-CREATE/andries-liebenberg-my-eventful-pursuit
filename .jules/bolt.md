...
## 2024-05-24 - [rAF Debouncing Ticking Flag Bug]
**Learning:** When using a `ticking` flag to debounce high-frequency events with `requestAnimationFrame`, setting `ticking = false` synchronously outside the callback or nesting rAF calls completely nullifies the debounce, causing the event to run on every trigger.
**Action:** Always ensure the `ticking` flag is strictly reset to `false` ONLY inside the async `requestAnimationFrame` callback. Avoid nested rAF calls for a single debounce loop.
