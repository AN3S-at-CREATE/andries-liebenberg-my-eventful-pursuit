...
## 2024-05-18 - [Fix nested requestAnimationFrame debouncing]
**Learning:** Over-nesting `requestAnimationFrame` calls and modifying the `ticking` flag synchronously outside the callback nullifies the debounce benefits, leading to layout thrashing and UI jank.
**Action:** Implement a single `requestAnimationFrame` layer and strictly reset the `ticking` flag to `false` inside the async callback itself.
