...
## 2026-04-02 - [Scroll Listener Layout Thrashing]
**Learning:** Using synchronous logic inside continuous event listeners (like `window.addEventListener("scroll", ...)`) causes severe layout thrashing and main-thread blocking, which leads to high CPU usage and degraded scrolling performance. Additionally, not marking these listeners as `{"passive": true}` blocks the browser's native scrolling mechanisms.
**Action:** Always wrap state updates inside synchronous event listeners using `window.requestAnimationFrame()` to debounce the execution. For continuous user-input events like scrolling or touch movements, ensure the listener is registered with `{"passive": true}` to permit the browser to handle scrolling natively without waiting for JavaScript execution.
