...
## 2024-05-14 - Prevent Layout Thrashing & Main Thread Blocking on Window Resize
**Learning:** Found multiple instances (`BackgroundFX.tsx`, `ParallaxElements.tsx`, `ParallaxStarfield.tsx`) where `window.addEventListener("resize")` handlers were synchronously evaluating mobile state, recalculating component layouts, or even re-initializing entire HTML5 Canvas particle systems on every single pixel resized. This forces excessive Garbage Collection, immediate synchronous reflows, and extreme main thread blocking resulting in heavy UI jank.
**Action:** Always debounce window `resize` event handlers using a `setTimeout` of ~150ms before triggering DOM reads (like `innerWidth`), setting state, or performing heavy layout recalculations (like `canvas.width` assignment). Make sure to `clearTimeout` properly within the resize handler itself and in the `useEffect` cleanup function.
