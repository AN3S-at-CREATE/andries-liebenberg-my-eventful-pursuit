...
## 2026-03-23 - Rate Limiter IP Spoofing
...
## 2026-03-23 - Predictable ID Generation
**Vulnerability:** Insecure, predictable ID generation using `Date.now()` on the client-side.
**Learning:** Using `Date.now()` for client-side message IDs in `useConciergeChat.ts` introduces potential ID collisions and allows for predictability/enumeration. Cryptographic randomness ensures unique, non-sequential identifiers.
**Prevention:** Use `crypto.randomUUID()` for robust, cryptographically secure ID generation instead of timestamp-based logic for component and element IDs.
