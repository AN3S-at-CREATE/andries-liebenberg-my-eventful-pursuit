...
## 2025-02-27 - User Enumeration via Auth Error Messages
**Vulnerability:** The sign-up flow (`src/pages/Auth.tsx`) exposed whether an email address was already registered in the system by returning a specific "Account exists" toast message when Supabase returned an "already registered" error.
**Learning:** Returning specific error messages during authentication flows (like sign-up, password reset, or login) allows attackers to perform user enumeration. They can script requests with lists of emails to determine which users have active accounts on the platform, aiding in targeted phishing or credential stuffing attacks.
**Prevention:** Always return a generic success message (e.g., "Check your email to verify your account") during sign-up, regardless of whether the email already exists in the system or is entirely new. This ensures the application's response is indistinguishable to an attacker.
