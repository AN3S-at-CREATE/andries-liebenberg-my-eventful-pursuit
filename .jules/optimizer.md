...
## 2025-10-25 - Scroll Event Optimization
   **Bottleneck:** The `ScrollToTop` component attached an unthrottled, non-passive `scroll` event listener to the window. Every scroll event triggered a synchronous React state update, causing excessive re-renders and potential main thread blocking (layout thrashing).
   **Learning:** Native scrolling can fire dozens of events per second. React state updates are synchronous during these events, leading to high CPU usage.
   **Prevention:** Always debounce synchronous state updates inside `scroll` event listeners using `window.requestAnimationFrame()` and mark the event listener with `{ passive: true }` to avoid blocking the browser's native scrolling mechanism.
