...
## 2024-03-01 - [ParallaxStarfield & GlobalCursorGlow Optimizations]
**Bottleneck:** Duplicate constants and unresolved git merge conflicts were causing build failures. Rendering loop in Starfield had high string-parsing overhead from `ctx.fillStyle`, and `GlobalCursorGlow` conditionally called hooks based on `isMobile`.
**Learning:** Draw calls should be batched to minimize canvas state changes (e.g., `ctx.fillStyle`). Hook execution (like `useTransform`) must happen at the top level and cannot be placed behind early returns like `if (isMobile) return`.
**Prevention:** Always run `pnpm lint` and `pnpm build` locally after manually resolving merge conflicts, and follow the rule of hooks to ensure all react hooks are executed unconditionally.
