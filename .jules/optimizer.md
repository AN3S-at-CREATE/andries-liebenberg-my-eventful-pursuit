...
## 2025-03-24 - Unoptimized Scroll Listeners
   **Bottleneck:** The `ScrollToTop` component was using a synchronous scroll event listener without debouncing or marking it as passive. This blocked the main thread during scrolling, leading to potential layout thrashing and high CPU usage.
   **Learning:** Synchronous scroll listeners execute on the main thread and block rendering, causing jittery scrolling. Using `window.requestAnimationFrame()` ensures the state update runs right before the next repaint, while `{ passive: true }` explicitly tells the browser the listener won't call `preventDefault()`, allowing native scroll optimizations.
   **Prevention:** Always debounce synchronous scroll event listeners using `window.requestAnimationFrame()` and mark the event listener with `{ passive: true }` to avoid blocking the browser's native scrolling.
