...
## 2024-03-29 - Debounce scroll events using requestAnimationFrame
**Bottleneck:** Synchronous scroll event listener causing layout thrashing and high CPU usage.
**Learning:** Using `requestAnimationFrame` and `{ passive: true }` debounces scroll events and prevents blocking native scrolling.
**Prevention:** Always debounce continuous event listeners like `scroll` and use `{ passive: true }` to avoid performance degradation.
