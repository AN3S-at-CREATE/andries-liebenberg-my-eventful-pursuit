...
## 2025-10-24 - Main Thread Blocking Scroll Events
   **Bottleneck:** The `ScrollToTop` component bound a synchronous state-updating event listener directly to the window `scroll` event. This blocks the main thread during scrolling, leading to layout thrashing, high CPU usage, and poor scroll performance.
   **Learning:** Synchronous operations on continuous events like `scroll` or `resize` quickly overload the browser's render cycle. Using `{ passive: true }` on the event listener explicitly tells the browser that the listener will not call `preventDefault()`, allowing native scroll to run smoothly. Combining this with `requestAnimationFrame` debounces the UI updates to match the display refresh rate.
   **Prevention:** Always debounce continuous synchronous event listeners (like `scroll`, `mousemove`, `resize`) using `window.requestAnimationFrame()` and mark the event listener with `{ passive: true }` when applicable to avoid blocking the browser's native rendering.
