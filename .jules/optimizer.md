...
## 2026-03-17 - Scroll Event Listener Optimization
   **Bottleneck:** The `window.addEventListener("scroll", ...)` in `ScrollToTop.tsx` was running synchronously on every scroll tick without debouncing and without the `{ passive: true }` flag, potentially causing main thread blocking and layout thrashing.
   **Learning:** Synchronous execution of DOM reads/writes during continuous scroll events can overwhelm the main thread. Debouncing with `window.requestAnimationFrame()` ensures the handler only executes once per display frame. Adding `{ passive: true }` tells the browser that the listener will not call `preventDefault()`, allowing it to perform native scrolling immediately without waiting for the JavaScript execution to finish.
   **Prevention:** Always debounce synchronous `scroll` event listeners using `window.requestAnimationFrame()` and mark the event listener with `{ passive: true }` to avoid blocking the browser's native scrolling and ensure smooth 60fps performance.
