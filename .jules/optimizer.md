...
## 2025-02-19 - Debounced Scroll Event Listeners
   **Bottleneck:** The scroll event listener in `ScrollToTop.tsx` fired synchronously on every scroll frame. This caused continuous state updates without debouncing, blocking the main thread and leading to layout thrashing and scroll jank, especially on mobile devices.
   **Learning:** The browser can fire scroll events much faster than it can repaint the screen. React state updates triggered synchronously on these events can overwhelm the main thread.
   **Prevention:** To prevent layout thrashing and high CPU usage during continuous scrolling, always debounce synchronous `scroll` event listeners using `window.requestAnimationFrame()` and mark the event listener with `{ passive: true }` to avoid blocking the browser's native scrolling.
