...
## 2026-03-19 - Unresolved Git Merge Conflicts Breaking Build
   **Bottleneck:** The components `ParallaxStarfield.tsx` and `GlobalCursorGlow.tsx` contained unresolved Git merge conflict markers (e.g. `<<<<<<<`, `=======`, `>>>>>>>`), which caused the Vite esbuild process to fail entirely and break the application build. This was preventing any code from being tested or deployed.
   **Learning:** Automated or manual merge processes can sometimes leave behind conflict markers if not verified properly. A broken build prevents downstream CI/CD pipelines and manual verifications.
   **Prevention:** Always verify build success (`pnpm build`) and linting (`pnpm lint`) after resolving Git merge conflicts. If attempting automated code modifications via script or search/replace tools, do not leave dangling markers and confirm changes against source before committing.
