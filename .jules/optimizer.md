...
## 2025-02-19 - Unoptimized Scroll Listeners
   **Bottleneck:** Raw \`scroll\` event listeners updating component state directly on every scroll event, leading to frequent React re-renders, layout thrashing, and high CPU usage.
   **Learning:** The \`scroll\` event fires synchronously and significantly more often than the browser's refresh rate (typically 60 times a second). Blocking the main thread with state updates on every scroll tick degrades scrolling performance.
   **Prevention:** Always debounce synchronous \`scroll\` event listeners using \`window.requestAnimationFrame()\` to match the display refresh rate. Also, pass \`{ passive: true }\` when attaching the listener so the browser knows \`preventDefault()\` won't be called, allowing smoother native scrolling.
