...
import { Calculator, TrendingUp, Clock, Target, Phone } from "lucide-react";
...
export function ROICalculatorModal({ trigger }: ROICalculatorModalProps) {
  ...
  // Input states
  const [currentRevenue, setCurrentRevenue] = useState(100000); // R100k
  const [investmentAmount, setInvestmentAmount] = useState(20000); // R20k
  const [expectedGrowth, setExpectedGrowth] = useState(15); // 15%
  const [timeframe, setTimeframe] = useState(6); // 6 months
