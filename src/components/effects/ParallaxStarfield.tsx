...
export function ParallaxStarfield() {
  ...
  const starsRef = useRef<{ cyan: Star[]; pink: Star[]; white: Star[] }>({ cyan: [], pink: [], white: [] });
  ...
  useEffect(() => {
    ...
    const initStars = () => {
      ...
      for (let i = 0; i < starCount; i++) {
        ...
        starsRef.current[color].push(star);
      }
    };
    ...
    const animate = (currentTime: number) => {
      ...
      // Iterate over color groups to batch draw calls
      (Object.keys(starsRef.current) as Array<keyof typeof STAR_COLORS>).forEach((colorKey) => {
        const stars = starsRef.current[colorKey];
        if (stars.length === 0) return;

        // Set fill style once for the entire group
        ctx.fillStyle = STAR_COLORS[colorKey];
        ...
        stars.forEach((star) => {
            ...
            if (viewportY > -50 && viewportY < window.innerHeight + 50) {
                ...
                // Only for non-white stars > size 1
                if (!isReducedMode && star.size > 1 && colorKey !== "white") {
      ...
      // Reset globalAlpha for safety (though cleared next frame)
      ctx.globalAlpha = 1.0;
