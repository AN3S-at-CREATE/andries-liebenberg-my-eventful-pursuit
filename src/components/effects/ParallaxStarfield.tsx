...
export function ParallaxStarfield() {
  ...
  const starsRef = useRef<{ white: Star[]; cyan: Star[]; pink: Star[] }>({
    white: [],
    cyan: [],
    pink: [],
  });
  ...
  useEffect(() => {
    ...
    const initStars = () => {
      ...
      starsRef.current = { white: [], cyan: [], pink: [] };
    ...
    const animate = (currentTime: number) => {
      ...
      (
        Object.keys(starsRef.current) as Array<keyof typeof starsRef.current>
      ).forEach((color) => {
        ctx.fillStyle = COLORS[color];
        starsRef.current[color].forEach((star) => {
