...
export const ScrollToTop = () => {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      // 🚀 Optimizer: Correctly implement requestAnimationFrame debounce
      // Ensures scroll events are checked at max 60fps without layout thrashing
      if (!ticking) {
        window.requestAnimationFrame(() => {
          setIsVisible(window.scrollY > 300);
          ticking = false; // Strictly reset inside callback
        });
        ticking = true;
      }
    };
